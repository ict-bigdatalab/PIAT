...
from marcopassage.dataset import MSMARCODataset_white_attack as MSMARCODataset_file_qd, get_collate_function, CollectionDataset
...
def read_resources(embed_path, embed_cos_matrix_path, embed_name,
    ...
    return synonym_helper, word_re, attack_qds, collection # ori_ranked_list_qds,
...
def run_parse_args():
    ...
    parser.add_argument("--data_num_workers", default=7, type=int)
    ...
    parser.add_argument("--simi_candi_topk", type=int, default=40)

    parser.add_argument("--simi_threshod", type=float, default=0.5)
...
def attack_by_testing_blackbox(ori_model, surr_model, batch_list, docid_list,
    ...
    # attacked_we_matrix_detach = attacked_we_matrix.clone().detach()
    ...
    new_doc_token_id_list, score, rank = \
    ...
    return new_doc_token_id_list, score, rank
...
def main():
    ...
    synonym_helper, word_recover, attack_qds, collection = read_resources \
    ...
    attacked_docs_rank_dict = {}
    # attacked_embedding_weight_dict = {}
    ...
    qdsf = open(args.black_model_ranked_list_score_path, 'r', encoding='utf-8')
    tested_q_num = 0
    for qid in qid_list_t:
        ...
        for count, line in enumerate(qdsf, 1):
            ss = line.strip().split('\t')
            qid = int(ss[0])
            did = int(ss[1])
            score = float(ss[2])
            if qid not in ori_qds:
                ori_qds[qid] = {}
            ori_qds[qid][did] = score
            if count == 100:
                break
        ...
        for attack_docid in attack_docid_list:

            surr_model.load_state_dict(surr_model_state_dict)
            ori_score = attack_qds[qid][attack_docid]
            new_doc_token_id_list, score, rank = attack_by_testing_blackbox(ori_model, surr_model,
                                                                      batch_list,
                                                                      docid_list,
                                                                      attack_docid, args,
            ...
            attacked_docs_rank_dict[attack_doc_key] = rank
            # attacked_embedding_weight_dict[attack_doc_key] = word_emb_weight

        # if tested_q_num % 100 == 0:
        ...
        #         to_write = qid_docid + '\t' + attacked_doc + '\t' + str(attacked_docs_score_dict[qid_docid]) + '\t' + str(attacked_docs_rank_dict[qid_docid])
        #         save_attacked_docs_f.write(to_write + '\n')

            # np.save('results/embed/attacked_embedding_weight_dict_{}.npy'.format(str(tested_q_num)),attacked_embedding_weight_dict)
            # attacked_embedding_weight_dict = {}
            attacked_docs_dict = {}
            attacked_docs_score_dict = {}
            attacked_docs_rank_dict = {}
